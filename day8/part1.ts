...
// O(n^2 . logn) time
function getDistances(coordinates: number[][], limit: number): PointDistance[] {
...
const distances = getDistances(input, 1000);

const unionRanks = unionize(distances);

console.log(unionRanks.slice(0,3).reduce((acc, cur) => acc * cur, 1))
